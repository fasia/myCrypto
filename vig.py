...
def encrypt(key, plaintext):
    ...
    for pair in pairs:
        total = reduce(lambda x, y: ord(x) + ord(y), pair)
        result += chr(total % 256)
...
def show_result(plaintext, key):
    ...
    print 'encrypt: %s' % encrypted
    print 'Decrytped: %s' % decrypted


if __name__ == '__main__':
    show_result('********************** Hello! Faezeh siavashi you won!**********************', 'keyword')
    print open('file2-35121.txt.enc', 'r').read()
